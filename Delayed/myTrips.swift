...
class myTrips: UIViewController {
    ...
    @IBOutlet weak var noSubsLabel: UILabel!
    
    @IBAction func addSubsButtonAction(_ sender: Any) {
        //return to all trips page
        tabBarController?.selectedIndex = 0
    }
    override func viewDidLoad() {
        super.viewDidLoad()
        
        addSubsButton.layer.cornerRadius = 20
        
        if(!userHasSubs) {
            tableView.isHidden = true
            addSubsButton.isHidden = false
            noSubsLabel.isHidden = false
            //self.navigationItem.rightBarButtonItem = self.editButtonItem
        }
        else {
            tableView.isHidden = false
            addSubsButton.isHidden = true
